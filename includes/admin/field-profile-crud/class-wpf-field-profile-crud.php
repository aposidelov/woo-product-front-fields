...
class WPF_Field_Profile_CRUD {    
  ...
  public function __construct( $plugin_name, $version ) {
    ...
    $this->retrive_field_profiles();
  }  
  /**
   * Retrive all field profiles      
   */   
  public function retrive_field_profiles() {
    $this->field_profiles = wpf_get_field_profiles();        
  }    
  ...
  public function enqueue_styles( $hook ) {                
    if ( isset( $_GET['page'] ) && ( 
         'wpf_field_profile_add' == $_GET['page'] || 
         'wpf_field_profile_edit' == $_GET['page'] ) ) {
  ...
  public function admin_menu_pages() {
    ...
    add_submenu_page( self::PARENT_MENU, $list_page_name, $list_page_name, 'edit_pages', self::LIST_PAGE, array( $this, 'list_menu_page_handler' ) );
    $add_form_name = __( 'Add field profile', 'wpf' );
    add_submenu_page( null, $add_form_name, $add_form_name, 'edit_pages', self::ADD_PAGE, array( $this, 'add_form_menu_page_handler' ) );
    $edit_form_name =  __( 'Edit field profile', 'wpf' );
    add_submenu_page( null, $edit_form_name, $edit_form_name, 'edit_pages', self::EDIT_PAGE, array( $this, 'edit_form_menu_page_handler' ) );
  }
  ...
  public function list_menu_page_handler() {                        
    ...
    <div class="wrap">
      ...
      <table class="wp-list-table wpf-field-profiles widefat fixed striped">
        ...
        <tbody>  
        <?php if ( sizeof( $this->field_profiles ) ) : ?>
        <?php foreach ( $this->field_profiles as $item ) : ?>
  ...
  public function edit_form_menu_page_handler() {
    echo do_shortcode( "[" . self::FORM_SHORTCODE . " id='{$_GET['id']}']" );
  }
  ...
  public function form() {
    $cmb = new_cmb2_box( array(
      ...
      'hookup'       => false,
      'save_fields'  => false, 
      'classes' => array( isset( $_GET['id'] ) ? 'wpf-edit-form' : 'wpf-add-form' ),      
    ) ); 
    if ( isset( $_GET['id'] ) ) {
      $cmb->add_field( array(            
        'id'      => 'id',    
    ...
    $cmb->add_field( array(
      ...
      'attributes' => array(
        'required' => 'required',        
        'data-form-type' => isset( $_GET['id'] ) ? 'edit' : 'add'
      ),
    ) );   
    $cmb->add_field( array(        
      'id'      => 'saved_name',    
    ...
    $fields = array_column( wpf_get_fields(), 'title', 'id' );
    $cmb->add_field( array(
      'name'    => 'Fields',        
      'id'      => 'fields',
      'type'    => 'select',
      'options' => array( '' => '-- Select a field --' ) + $fields,
  ...
  public function form_submit() {
    ...
    $edit_form = isset( $_GET['id'] );
    $except_current = $edit_form ? $_POST['saved_name'] : '';    
    if ( !wpf_is_profile_name_unique( $_POST['name'], $except_current ) ) {
    ...
    $this->edit_page_redirect( $id, true );
  }
  ...
  public function form_shortcode( $atts = array() ) {        
    ...
    if ( isset( $id ) && !empty( $id ) && ! isset( $_POST['name'] ) ) {         
      $field_item = $this->field_profiles[ $id ];                        
  ...
  public function field_profiles_links() {        
    ...
    foreach ( $this->field_profiles as $id => $field_profile ) {
      $name = ucwords( $field_profile['name'] );
      $name = __( "Add New {$name}", 'wpf' );      
      add_submenu_page( 'edit.php?post_type=product', $name, $name, 'edit_products', 'post-new.php?post_type=product&fpid='. $id ); 
    }          
  }  
